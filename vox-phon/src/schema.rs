...
pub fn parse_schema_bytes(bytes: &[u8]) -> Result<SchemaBundle, Error> {
    ...
    for _ in 0..count {
        ...
        // `schema_from_bytes`'s second tuple element is the owned-allocation
        // byte count charged against `DecodeLimits` (string/Vec bytes it had
        // to copy), not a buffer position — it is unrelated to `slice.len()`
        // and will not equal it for any schema with structural framing
        // (struct/enum tags, field names, list-length headers). Full-slice
        // consumption is already guaranteed by `schema_from_bytes` itself,
        // which rejects trailing bytes internally before returning `Ok`.
        let (schema, _owned_bytes_charged) = schema_from_bytes(slice, DecodeLimits::DEFAULT)
