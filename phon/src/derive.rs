...
pub fn of_shape(shape: &'static Shape) -> Result<Derived, DeriveError> {
    ...
    // A parse/display opaque scalar root (Uuid, chrono, url, …): a
    // `Primitive::Bytes` run carrying the value's `Display` string; decoded via
    // the shape's `parse` vtable entry. (FTS vendor addition.)
    if display_scalar(shape) {
        return Ok(Derived {
            root: primitive_id(Primitive::Bytes),
            schemas: Vec::new(),
            descriptor_blocks: HashMap::new(),
            descriptor: display_scalar_descriptor(shape)?,
        });
    }
...
impl Builder {
    ...
    fn ref_of(&mut self, shape: &'static Shape) -> Result<SchemaRef, DeriveError> {
        ...
        if let Some(p) = scalar_primitive(shape)? {
            ...
            // (FTS vendor addition.)
            Ok(SchemaRef::concrete(primitive_id(Primitive::Bytes)))
        } else {
...
fn build_descriptor(
    ...
    // A parse/display opaque scalar field (Uuid, chrono, url, …).
    // (FTS vendor addition.)
    if display_scalar(shape) {
        return display_scalar_descriptor(shape);
    }
...
// ============================================================================
// Parse/display opaque scalars (Uuid, chrono, url, …) — FTS vendor addition
// ============================================================================
//
// Facet models third-party scalar types (uuid::Uuid, chrono::DateTime,
// url::Url, …) as `Type::User(UserType::Opaque)` + `Def::Scalar` shapes whose
// vtable carries `display` and `parse`. Upstream phon rc.5 has no branch for
// them, so deriving any struct containing one fails — and because the opaque
// encode thunk panics on a derive failure, an RPC reply containing e.g. a
// `Uuid` field aborted the whole server.
//
// This bridge encodes such a scalar as a `Primitive::Bytes` run (the engine
// frames a `u32` length) whose content is the value's `Display` string in
// UTF-8; decode parses it back through the shape's `parse` vtable entry. Both
// sides derive from the same shape, so writer and reader agree on the wire.
// Precedence: every specific branch (fixed scalars, String, containers, real
// opaque adapters) is checked first — this is the last resort before the
// "cannot derive" error.

/// Whether `shape` is a parse/display opaque scalar: `Def::Scalar` with both
/// `display` and `parse` in its vtable, and not a fixed-width primitive (those
/// take the `scalar_primitive` branch first).
fn display_scalar(shape: &'static Shape) -> bool {
    matches!(shape.def, Def::Scalar) && shape.vtable.has_display() && shape.vtable.has_parse()
}

/// The descriptor for a parse/display scalar: schema `Primitive::Bytes`, an
/// [`Access::Opaque`] whose thunks stringify/parse through the shape's vtable.
/// The `ctx` is the `&'static Shape` itself.
fn display_scalar_descriptor(shape: &'static Shape) -> Result<Descriptor, DeriveError> {
    let (size, align) = layout_of(shape)?;
    Ok(Descriptor {
        schema: SchemaRef::concrete(primitive_id(Primitive::Bytes)),
        layout: Layout { size, align },
        access: Access::Opaque(OpaqueThunks {
            ctx: core::ptr::from_ref(shape).cast::<()>(),
            encode: display_scalar_encode,
            decode: display_scalar_decode,
        }),
    })
}

/// `Display` adapter over a raw `(shape, ptr)` pair, so `format!`/`to_string`
/// can drive the shape's `display` vtable entry.
struct DisplayViaShape {
    shape: &'static Shape,
    ptr: PtrConst,
}

impl fmt::Display for DisplayViaShape {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Safety: `ptr` points at a live value of `shape` for the duration of
        // the encode thunk that constructed us.
        match unsafe { self.shape.call_display(self.ptr, f) } {
            Some(r) => r,
            None => Err(fmt::Error),
        }
    }
...
/// [`OpaqueThunks::encode`] for a parse/display scalar: append the value's
/// `Display` string (UTF-8) to `out`. The engine frames the `u32` length.
///
/// # Safety
/// `ctx` must be the `&'static Shape` set in [`display_scalar_descriptor`];
/// `field` must point to an initialized value of that shape; `out` must be a
/// live `Vec<u8>`.
unsafe extern "C" fn display_scalar_encode(ctx: *const (), field: *const u8, out: *mut Vec<u8>) {
    // Safety: `ctx` is the `&'static Shape`.
    let shape: &'static Shape = unsafe { &*ctx.cast::<Shape>() };
    let s = DisplayViaShape {
        shape,
        ptr: PtrConst::new(field),
    }
    .to_string();
    // Safety: `out` is a live `Vec<u8>`.
    unsafe { (*out).extend_from_slice(s.as_bytes()) };
}

/// [`OpaqueThunks::decode`] for a parse/display scalar: parse the UTF-8 span
/// back through the shape's `parse` vtable entry into `slot`. Returns `false`
/// (a decode error, not a panic) on invalid UTF-8 or a parse failure.
///
/// # Safety
/// `ctx` must be the `&'static Shape` set in [`display_scalar_descriptor`];
/// `bytes[..len]` must be a readable span; `slot` must be uninitialized storage
/// of that shape's size and alignment.
unsafe extern "C" fn display_scalar_decode(
    ctx: *const (),
    bytes: *const u8,
    len: usize,
    slot: *mut u8,
    ...
    // Safety: `bytes[..len]` is a readable span borrowed from the reader's input.
    let span = unsafe { core::slice::from_raw_parts(bytes, len) };
    let Ok(s) = core::str::from_utf8(span) else {
        return false;
    };
    // Safety: `slot` is uninitialized storage for the scalar; `parse`
    // initializes it on `Ok`.
    matches!(
        unsafe { shape.call_parse(s, PtrUninit::new(slot)) },
        Some(Ok(()))
    )
}
