...
///
/// FTS vendored fix: `<` / `>` are also quoted — the styx-parse lexer
/// rejects bare scalars containing angle brackets (e.g. keybind strings
/// like `<C-s>`), so emitting them bare produced files that could not
/// be re-parsed.
